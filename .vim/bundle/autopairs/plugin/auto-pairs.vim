...
function! AutoPairsInsert(key)
  ...
  " skipping apostrophes
  "if current_char == close && open == close
	"return "\<Right>"
  "end
  " clime:
  if current_char == close && open == close
	if delimitMate#GetSyntaxRegion(line('.'), col('.')-1) == 'Constant' || delimitMate#GetSyntaxRegion(line('.'), col('.')-1) == 'String'
		return "\<Right>"
	endif
	return a:key
  end
  ...
  " clime: set of restrictions START
  if (a:key == "'" || a:key == '"') && 
		\ ((prev_char !~ '\s' && prev_char != '') || (current_char !~ '\s' && current_char != ''))
	  return a:key
  end
 
  if (a:key == "{") && (current_char =~ '\v\w')
    return a:key
  end

  if (a:key == "(" || a:key == "[") && (current_char !~ '\s' && current_char != '')
    return a:key
  end

  " clime: adding excluded-regions-functionality from delimitMate
  "let b:_l_delimitMate_excluded_regions_enabled = 1
  "let b:_l_delimitMate_excluded_regions = 'String,Constant'
  "let b:_l_delimitMate_excluded_regions_list = split(b:_l_delimitMate_excluded_regions, ',\s*')
  "if delimitMate#IsForbidden(a:key) 
	  "return a:key
  "end
  " clime: set of restrictions END
